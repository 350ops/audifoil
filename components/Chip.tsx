...
export const Chip = ({
    ...
    if (icon) {
      return (
        <Icon 
          ...
          color={iconColor || (isChipSelected ? colors.invert : colors.icon)} 
          className="mr-2 -ml-1" 
        />
      );
    }
  ...
  const chipWrapper = (children: ReactNode) => (
      ...
      <View
        className={`${className} ${paddingClasses} rounded-full ${isChipSelected ? 'bg-highlight' : 'bg-secondary'} flex-row items-center justify-center ${bordered ? 'border border-border' : ''}`}
        style={style}
      >
        {children}
      </View>
