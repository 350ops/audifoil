            ...
            <View className="p-4">
                {options.map((option) => (
                    <Pressable
                        ...
                        className={`py-3 px-4 rounded-lg mb-2 ${selectedOption?.value === option.value ? inputBgColor : ''}`}
                    >
                        <ThemedText>
                            {option.label}
                        </ThemedText>
        ...
        return (
            <View className={`mb-global relative ${containerClassName}`} style={style}>
                ...
                <TouchableOpacity
                    onPress={handlePress}
                    disabled={disabled}
                    className={`${inputBgColor} rounded-lg px-global h-16 flex-row justify-between items-center
                        ${isFocused ? borderColor : 'border-border/60'}
            ...
            <View className={`mb-global relative ${containerClassName}`} style={{ position: 'relative', ...style }}>
                ...
                <View className="relative">
                    <TouchableOpacity
                        onPress={handlePress}
                        disabled={disabled}
                        className={`border ${inputBgColor} rounded-xl px-3 h-14  text-text flex-row justify-between items-center
            ...
            <View className={`mb-global relative ${containerClassName}`} style={{ position: 'relative', ...style }}>
                <View className="relative">
                    ...
                    <TouchableOpacity
                        onPress={handlePress}
                        disabled={disabled}
                        className={`border-b-2 py-3 px-0 h-14 pr-0 text-text bg-transparent border-t-0 border-l-0 border-r-0 flex-row justify-between items-center
                            ${isFocused ? borderColor : 'border-border'}
                            ${error ? 'border-red-500' : ''}
                            ${disabled ? 'opacity-50' : ''}
                            ${className}`}
                    >
